...
def read_in_file():
    catalog = pd.read_csv('/Users/xinyue/PycharmProjects/cic_peoject/2004/2004.csv')
    catalog = clean_space(catalog)
    catalog.to_csv('checking_2004.csv')
    clean_catalog_number = clean_number(catalog)
    clean_catalog_number.to_csv('checking_2004.csv')
    ...
    clean_catalog_number.to_csv('2004_catalog.csv')

def clean_space(catalog):
    for i, r in catalog.iterrows():
        text = r['外商投资产业指导目录'].lstrip()
        catalog.at[i, '外商投资产业指导目录'] = text
    return catalog
        ...
        if '.' in r['外商投资产业指导目录']:
            ...
            if r['外商投资产业指导目录'][0] == '（':
                ...
                catalog.at[i, 'content'] = text2
                    ...
                    text2 = ('').join(text[1:])
                    text2 = text2.replace('　', '')
                    catalog.at[i, 'output_file'] = text1.replace('(','（') + ' )'
                    catalog.at[i, 'content'] = text2
