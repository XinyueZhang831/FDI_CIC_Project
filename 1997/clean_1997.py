...
def read_in_file():
    catalog = pd.read_csv('/Users/xinyue/PycharmProjects/cic_peoject/1997/1997_original.csv')
    ...
    clean_catalog_number.to_csv('1997_catalog.csv')
...
def clean_number(catalog):
    ...
    for i,r in catalog.iterrows():
        if '．' in r['鼓励外商投资产业目录']:
            ...
            catalog.at[i, 'output_file'] = r['鼓励外商投资产业目录']
            catalog.at[i, 'content'] = ''


        for item_2 in chinese_char_2:
            if item_2 in r['鼓励外商投资产业目录']:
                text = r['鼓励外商投资产业目录'].split('、')
                text1 = text[0]
                text2 = ('').join(text[1:])
                text2 = text2.replace('　', '')
...
def second_modeify(m_only):
    ...
    for i,r in m_only.iterrows():
        ...
        text_list = []
        for j in text:
            if ('中外' in j) |('中方' in j) |('外资' in j) |('合资' in j) |('合作' in j)|('外商' in j):#if any new key words, update here
                text_list.append(j)
                if (j in text_list_for_split) == False:
                    text_list_for_split.append('（' +j+ '）')
        m_only.at[i, 'restriction'] = (' ').join(text_list)
    #print(m_only['content'])
